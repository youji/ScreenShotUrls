...
const htmlDir = "./result/htmlcode/" + TimeId.getTimeId() + "/";
(async () => {
  if (conf.saveHtml) await fs.mkdir(htmlDir, { recursive: true });
})();

const assetDir = "./result/asset/" + TimeId.getTimeId() + "/";
(async () => {
...
async function saveAssetFile(page) {
  page.on("response", async (res) => {
    ...
    if (contentType.indexOf("text/html") != -1 && pathname.endsWith("/")) {
      pathname += "index.html";
    }
